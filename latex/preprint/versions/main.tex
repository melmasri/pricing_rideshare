...
\begin{figure}[h!tbp]
    ...
    \includegraphics[width=0.45\linewidth]{img/sampled travel times.png}
    \caption{(Left) Comparing the empirical cumulative distribution of the 2000 test trips, our proposed travel time sampling methods, and the HMM model of~\cite[Algo. 2]{woodard2017predicting}. (Right - placeholder) Route sampling method.}
...
Figure~\ref{fig:fidelity_traveltime}(right) showcases the empirical cumulative distribution of the sampled travel times alongside the 2000 test trips, when the route is varied as well for different values of $\tau = \cbr{0.1, 1, 2}$. High fidelity is observed for $\tau = ?$, and similarity, the higher the order the higher the sampling fidelity. 
...
Table~\ref{tab:strike-metrics} illustrate important assessments of different model types. [to finish]
\begin{table}[h!]
    \centering
    \caption{Model assessment for the trip-specific and the population models under different strike prices. Profit is in CAD}
    \begin{tabular}{l|l|c| c | c}
    Model & Strike $P_\path(\pstart, t_0)$   & \% profitable & Average profit & Maximum loss\\
    \hline
    Population & $\times 1$   &  & &  \\
    & $\times e^{0.05}$    &  & &  \\
     & $\times e^{0.1}$    &  & &  \\
    Trip-specific & $\times 1$   &  & &  \\
    & $\times e^{0.05}$    &  & &  \\
     & $\times e^{0.1}$    &  & &  \\
    \end{tabular}
...
\begin{table}[h!]
    \centering
    \caption{Model assessment for the trip-specific and the population models under different . Profit is in CAD}
    \begin{tabular}{l|l|c| c | c}
    Model & Cutoff rate $\zeta$   & \% profitable & Average profit & Maximum loss\\
    \hline
  Population  & $0$    &  & &  \\
     & $-0.1$    &  & &  \\
  Trip-specific & $0$    &  & &  \\
